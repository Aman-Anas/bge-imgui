...
def getIntegerNew(val: int, num: int = 1):
    buf = bgl.Buffer(bgl.GL_INT, num)
    bgl.glGetIntegerv(val, buf)
    return buf.to_list()
...
class ProgrammablePipelineRenderer(BaseOpenGLRenderer):
    ...
    def render(self, draw_data):
        ...
        last_program = getIntegerNew(gl.GL_CURRENT_PROGRAM)
        last_active_texture = getIntegerNew(gl.GL_ACTIVE_TEXTURE)
        last_array_buffer = getIntegerNew(gl.GL_ARRAY_BUFFER_BINDING)
        last_element_array_buffer = getIntegerNew(
            gl.GL_ELEMENT_ARRAY_BUFFER_BINDING)
        last_vertex_array = getIntegerNew(gl.GL_VERTEX_ARRAY_BINDING)
        ...
        # gl.glActiveTexture(bgl.GL_TEXTURE0)
        bgl.glPolygonMode(bgl.GL_FRONT_AND_BACK, bgl.GL_FILL)

        bgl.glViewport(0, 0, int(fb_width), int(fb_height))
        ...
        ortho_np = np.asarray(ortho_projection)
        #buf = np_array_as_bgl_Buffer(ortho_np)

        # gl.glUseProgram(self._shader_handle)
        #bgl.glUniform1i(self._attrib_location_tex, 0)
        # gl.glUniformMatrix4fv(self._attrib_proj_mtx, 1,
        #                       gl.GL_FALSE, ortho_projection)
        #bgl.glBindVertexArray(self._vao_handle)

        for commands in draw_data.commands_lists:
            idx_buffer_offset = 0

            bgl.glBindBuffer(gl.GL_ARRAY_BUFFER, self._vbo_handle)
            # todo: check this (sizes)

            arrayType = ctypes.c_int16 * 2000
            arr = ctypes.c_void_p(commands.vtx_buffer_data)
            nparr = np.frombuffer(
                ctypes.cast(arr, ctypes.POINTER(arrayType)))

            buf = bgl.Buffer(bgl.GL_BYTE, (1, *nparr.shape))[0]

            bgl.glBufferData(gl.GL_ARRAY_BUFFER, commands.vtx_buffer_size * imgui.VERTEX_SIZE,
                             buf, gl.GL_STREAM_DRAW)

            bgl.glBindBuffer(gl.GL_ELEMENT_ARRAY_BUFFER, self._elements_handle)
            # todo: check this (sizes)

            arrayType = ctypes.c_int16 * 2000
            arr = ctypes.c_void_p(commands.idx_buffer_data)
            nparr = np.frombuffer(
                ctypes.cast(arr, ctypes.POINTER(arrayType)))

            buf = bgl.Buffer(bgl.GL_BYTE, (1, *nparr.shape))[0]

            bgl.glBufferData(gl.GL_ELEMENT_ARRAY_BUFFER, commands.idx_buffer_size *
                             imgui.INDEX_SIZE, buf, gl.GL_STREAM_DRAW)

            # todo: allow to iterate over _CmdList
            for command in commands.commands:
                bgl.glBindTexture(gl.GL_TEXTURE_2D, command.texture_id)

                # todo: use named tuple
                x, y, z, w = command.clip_rect
                bgl.glScissor(int(x), int(fb_height - w),
                              int(z - x), int(w - y))
                ...
                arrayType = ctypes.c_int16 * 2000
                arr = ctypes.c_void_p(idx_buffer_offset)
                nparr = np.frombuffer(
                    ctypes.cast(arr, ctypes.POINTER(arrayType)))

                buf = bgl.Buffer(bgl.GL_BYTE, (1, *nparr.shape))[0]

                bgl.glDrawElements(gl.GL_TRIANGLES, command.elem_count,
                                   gltype, buf)
        ...
        bgl.glActiveTexture(last_active_texture[0])
        bgl.glBindVertexArray(last_vertex_array[0])
        bgl.glBindBuffer(gl.GL_ARRAY_BUFFER, last_array_buffer[0])
        bgl.glBindBuffer(gl.GL_ELEMENT_ARRAY_BUFFER, last_element_array_buffer[0])
...
def get_common_gl_state():
    ...
    last_texture = getIntegerNew(bgl.GL_TEXTURE_BINDING_2D)
    last_viewport = getIntegerNew(bgl.GL_VIEWPORT)
    last_enable_blend = getIntegerNew(bgl.GL_BLEND)
    last_enable_cull_face = getIntegerNew(bgl.GL_CULL_FACE)
    last_enable_depth_test = getIntegerNew(bgl.GL_DEPTH_TEST)
    last_enable_scissor_test = getIntegerNew(bgl.GL_SCISSOR_TEST)
    last_scissor_box = getIntegerNew(bgl.GL_SCISSOR_BOX)
    last_blend_src = getIntegerNew(bgl.GL_BLEND_SRC)
    last_blend_dst = getIntegerNew(bgl.GL_BLEND_DST)
    last_blend_equation_rgb = getIntegerNew(bgl.GL_BLEND_EQUATION_RGB)
    last_blend_equation_alpha = getIntegerNew(bgl.GL_BLEND_EQUATION_ALPHA)
    last_front_and_back_polygon_mode, _ = getIntegerNew(
        bgl.GL_POLYGON_MODE, 2)
    return (
        last_texture,
        last_viewport,
        last_enable_blend,
...
def restore_common_gl_state(common_gl_state_tuple):
    ...
    #bgl.glBindTexture(gl.GL_TEXTURE_2D, last_texture[0])
    # bgl.glBlendEquationSeparate(
    ...
    # bgl.glScissor(last_scissor_box[0], last_scissor_box[1],
    #              last_scissor_box[2], last_scissor_box[3])
    # bgl.glViewport(last_viewport[0], last_viewport[1],
